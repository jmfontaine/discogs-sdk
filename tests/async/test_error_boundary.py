...
class TestNonJsonFailures:
    ...
    async def test_paginated_read(self, client, respx_mock):
        ...
        with pytest.raises(DiscogsAPIError) as exc_info:
            async for _ in client.users.get("trent_reznor").collection.folders.get(0).releases.list():
                pass
    ...
    async def test_empty_403_body_keeps_specialized_error(self, client, respx_mock):
        ...
        with pytest.raises(ForbiddenError) as exc_info:
            async for _ in client.users.get("atticus_ross").collection.folders.get(1).releases.list():
                pass
...
class TestSuccessfulResponsesAreUntouched:
    ...
    async def test_paginated_success(self, client, respx_mock):
        ...
        items = [item async for item in client.users.get("trent_reznor").collection.folders.get(0).releases.list()]

        assert len(items) == 1

    async def test_malformed_success_body_is_not_replaced_by_a_fallback(self, client, respx_mock):
