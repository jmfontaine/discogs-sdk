...
class TestLabelModel:
    async def test_required_fields(self, client, respx_mock):
        respx_mock.get("/labels/1").mock(return_value=respx.MockResponse(200, json={"id": 1, "name": "L"}))
        result = await client.labels.get(1)
        assert result.id == 1

    async def test_extra_allow(self, client, respx_mock):
        respx_mock.get("/labels/1").mock(
            return_value=respx.MockResponse(200, json={"id": 1, "name": "L", "_unknown_extra_field": "test"})
        )
        result = await client.labels.get(1)
